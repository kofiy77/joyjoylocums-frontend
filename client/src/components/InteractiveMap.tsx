...
interface InteractiveMapProps {
  ...
  postcode?: string;
  className?: string;
  // New props for GP practices
  practiceName?: string;
  practiceAddress?: string;
  practicePostcode?: string;
}

// Comprehensive UK postcode mapping for medical practices
const getCoordinatesFromPostcode = (postcode?: string): [number, number] => {
  // Default to London center
  const londonCenter: [number, number] = [51.5074, -0.1278];
  
  // Return default if no postcode provided
...
const InteractiveMap: React.FC<InteractiveMapProps> = ({
  ...
  // Use GP practice data or fallback to care home data
  const displayName = practiceName || careHomeName || 'Medical Practice';
  const displayAddress = practiceAddress || address || 'Address not available';
  const displayPostcode = practicePostcode || postcode || 'Postcode not available';
  
  const coordinates = getCoordinatesFromPostcode(displayPostcode);
        ...
        <Marker position={coordinates}>
          <Popup>
            <div className="text-center">
              <div className="font-semibold text-medical-blue">{displayName}</div>
              <div className="text-sm text-gray-600 mt-1">{displayAddress}</div>
              <div className="text-sm font-medium text-gray-800">{displayPostcode}</div>
            </div>
          </Popup>
        </Marker>
      </MapContainer>
